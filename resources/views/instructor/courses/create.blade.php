    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <form method="POST" action="{{ route('instructor.courses.store') }}" class="space-y-5 rounded-xl border border-slate-200 bg-white/85 p-6">
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="5" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-red-700 focus:ring-red-700" required>{{ old('description') }}</textarea>
            ...
            <label class="inline-flex items-center gap-2 text-sm text-slate-700">
                <input type="checkbox" name="is_published" value="1" class="rounded border-slate-300 text-red-800 focus:ring-red-700" @checked(old('is_published'))>
                Publish immediately
            </label>
